...
public class KillMulOne extends Scanner {
  ...
  public Expr visitMul(Mul mul) {
    ...
    if (isOne(mul.left)) {
      return mul.right.visit(this);
    }
    if (isOne(mul.right)) {
      return mul.left.visit(this);
    }

    if (isMinisOne(mul.left)) {
      return minis(mul.right.visit(this));
    }
    if (isMinisOne(mul.right)) {
      return minis(mul.left.visit(this));
    }
  ...
  private boolean isOne(Expr expr) {
    if (expr instanceof Minis) {
      return isMinisOne(((Minis) expr).target);
    }
    if (expr instanceof ConstInt) {
      return ((ConstInt) expr).isOne();
    }
    return false;
  }
  ...
  private boolean isMinisOne(Expr expr) {
    if (expr instanceof Minis) {
      return isOne(((Minis) expr).target);
    }
    if (expr instanceof ConstInt) {
      return ((ConstInt) expr).isMinisOne();
    }
    return false;
  }
  ...
  private boolean isZero(Expr expr) {
    if (expr instanceof Minis) {
      return isZero(((Minis) expr).target);
    }
    if (expr instanceof ConstInt) {
